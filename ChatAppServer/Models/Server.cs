    ...
    internal class Server
    {
        ...
        public static void EcouterReseau(Socket clientSocket)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    if (bytesRead > 0)
                    {
                        ...
                        // viewModel.NewMessage($"{clients[clientSocket]}: {messageFromClient.Replace($"{clients[clientSocket]}:", "")}");
            ...
            catch (SocketException)
            {
                ...
                // viewModel.new($"{clients[clientSocket]} s'est déconnecté."); Dynamic only
